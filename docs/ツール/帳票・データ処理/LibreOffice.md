...
LibreOfficeは、完全無料のオープンソースオフィススイートで、基本設計フェーズでは帳票レイアウト設計、帳票テンプレート作成、PDF出力仕様の定義に活用します。Writer（ワープロ）、Calc（表計算）、Draw（図形描画）を組み合わせて、印刷帳票やPDF帳票の詳細設計を行います。
